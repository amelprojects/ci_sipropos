...
function add_training(project_id)
{
    save_method = 'add';
...
function save()
{
    ...
    if(save_method == 'add') {
        var url = "<?php echo site_url('project/ajax_add_training')?>";
    } else {
        var url = "<?php echo site_url('project/ajax_edit_training')?>";
    }
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            else
            {
                for (var i = 0; i < data.inputerror.length; i++) 
                {
                    ...
                    // new PNotify({title: 'Training',text: data.error_string[i], styling: 'bootstrap3'});
                }
            }
        ...
        {
            new PNotify({title: 'Training',type: 'error', text: "Ada kesalahan pada sistem kami", styling: 'bootstrap3'});
            $('#btnSave').text('save'); //change button text
            $('#btnSave').attr('disabled',false); //set button enable 
...
function edit_training(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('project/ajax_get_training')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
...
function delete_training(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('project/ajax_delete_training')?>/"+id,
            type: "POST",
            dataType: "JSON",
